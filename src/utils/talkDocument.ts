...
<body>
  <header>
    ...
    <p class="muted">Toolbox Talk Record · ${escapeHtml(talk.date || 'Date not set')}</p>
  </header>
  ...
  <section class="certification">
    <span class="label">Given and certified by</span>
    <p class="value">${escapeHtml(talk.approvedBy || talk.supervisor || 'Not signed')}</p>
    <p class="muted">${talk.approvedAt ? `Signed ${escapeHtml(new Date(talk.approvedAt).toLocaleString())}` : 'Awaiting human sign-off'} · Prepared with assistance, reviewed and approved by the person named above.</p>
  </section>
...
export const createTalkPdf = (talk: ToolboxTalk): jsPDF => {
  ...
  writeLines(['Toolbox Talk Record'], 9);
  pdf.setTextColor(26, 26, 26);
  y += 2;
  ...
  writeHeading('Given and Certified By');
  writeParagraph(`${talk.approvedBy || talk.supervisor || 'Not signed'}${talk.approvedAt ? ` — Signed ${new Date(talk.approvedAt).toLocaleString()}` : ' — Awaiting human sign-off'}`, 10);
  writeParagraph('Prepared with assistance, reviewed and approved by the person named above.', 9);
