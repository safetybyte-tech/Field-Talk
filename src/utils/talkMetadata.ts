...
type Metadata = Pick<ToolboxTalk, 'deliveryPending' | 'notes' | 'draftStep' | 'drafted' | 'approved' | 'approvedBy' | 'approvedAt' | 'approvedByUserId' | 'approvedRecord' | 'harness'>;
...
export function encodeTalkContent(talk: ToolboxTalk): string {
  const { deliveryPending, notes, draftStep, drafted, approved, approvedBy, approvedAt, approvedByUserId, approvedRecord, harness } = talk;
  return JSON.stringify({ fieldTalkRecordVersion: 1, content: talk.content,
    metadata: { deliveryPending, notes, draftStep, drafted, approved, approvedBy, approvedAt, approvedByUserId, approvedRecord, harness } });
}
  ...
  try {
    ...
    if (value?.fieldTalkRecordVersion === 1 && typeof value.content === 'string') {
      ...
      return { content: value.content, metadata: {
        deliveryPending: m.deliveryPending === true,
