...
export function parseHarnessReview(value: unknown): HarnessReview | undefined {
  ...
  return { version: v.version, retrieval: v.retrieval, validation: v.validation, persisted: v.persisted, editedSinceGeneration: v.editedSinceGeneration === true };
}
...
export function harnessMessages(review: HarnessReview): string[] {
  ...
  if (review.editedSinceGeneration) messages.push('Edited since generation: original source matching has not been rerun. Recheck the task, controls, and any remaining citations.');
