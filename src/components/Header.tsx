...
export const Header: React.FC<HeaderProps> = ({ user, onEditProfile, onShowOutbox, talks = [], onTitleClick }) => {
  ...
  return <>
    <header className="border-b border-rule bg-ground"><div className="mx-auto flex h-[60px] max-w-[760px] items-center justify-between px-5">
      <button onClick={onTitleClick} className="min-h-11 text-left leading-none" aria-label="Go to Field Talk home"><span className="block text-[19px] font-bold tracking-[-.025em] text-ink">Field Talk</span><span className="snd-label mt-1 block text-[9px] text-accent">Safety Net Dispatch</span></button>
    ...
    {!isOnline && <div className="border-b border-[#F3E2C7] bg-caution-tint px-5 py-3 text-sm text-caution-text"><div className="mx-auto max-w-[760px]">No signal. Keep this page open to retain your edits. Reconnect, then save or send again; nothing sends automatically.</div></div>}
  </>;
};
