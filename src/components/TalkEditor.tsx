...
import { harnessMessages, parseHarnessReview } from '../utils/harness';
...
export const TalkEditor: React.FC<TalkEditorProps> = ({ talk, onSave, onSubmit, recentNames, currentUser, onRemoveRecentName }) => {
  ...
  const writeItUp = async () => {
    ...
    try {
      ...
      const content = parseStructuredTalkContent(result.content) || makeStructured(result.content); const next = { ...editedTalk, title: editedTalk.title || `Safety Talk: ${notes.trim()}`, content: JSON.stringify(content), notes, drafted: true, approved: false, harness };
  ...
  const reviewMessages = editedTalk.harness ? harnessMessages(editedTalk.harness) : [];
  ...
  return <main className="mx-auto max-w-[760px] px-5 pb-32 pt-7">
    ...
    {editedTalk.harness && <aside aria-label="V2 draft review" className="mt-5 border border-rule bg-sheet p-4 text-sm text-ink-body">
