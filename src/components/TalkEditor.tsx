...
export const TalkEditor: React.FC<TalkEditorProps> = ({ talk, onSave, onSubmit, recentNames, currentUser, onRemoveRecentName }) => {
  ...
  const writeItUp = async () => {
    ...
    try {
      ...
      setStructured(content); setEditedTalkState(next); onSave({ ...next, draftStep: 1 }); logger.logEvent(currentUser?.id || '', editedTalk.id, 'talk_generated', { latency_ms: logger.getElapsedTime(`ai_generation_${editedTalk.id}`), generator: useHarnessV2 ? 'harness_v2' : 'v1' });
  ...
  const chooseTemplate = (id: string) => {
    ...
    setStructured(content); setEditedTalkState(next); onSave({ ...next, draftStep: 1 }); logger.logEvent(currentUser?.id || '', editedTalk.id, 'task_selected', { source: 'template_selection', template_id: id });
  };
  const proceed = async () => {
    if (sending) return;
    setError('');
