...
import { approvalSnapshot, changeRecord, clearApproval, hasCurrentApproval, reviewMessages as getReviewMessages, signOffError, signRecord } from '../utils/recordReview';
...
const draftVersion = (talk: ToolboxTalk, step = talk.draftStep || 1) => JSON.stringify([approvalSnapshot(talk), step, talk.attendees, talk.recipients, talk.approvedRecord, talk.approvedAt, talk.submittedAt]);
...
export const TalkEditor = React.forwardRef<TalkEditorHandle, TalkEditorProps>(function TalkEditor({ talk, onSave, onSubmit, recentNames, currentUser, onRemoveRecentName, onDone }, ref) {
  ...
  React.useEffect(() => setWarningsAcknowledged(false), [currentSnapshot]);
  const approved = hasCurrentApproval(editedTalk);
  ...
  React.useEffect(() => { if (error) errorRef.current?.scrollIntoView({ block: 'center', behavior: 'smooth' }); }, [error]);
  ...
  React.useEffect(() => { setEditedTalkState(talk); setStep(talk.draftStep || 1); setStructured(parseStructuredTalkContent(talk.content)); }, [talk]);
  ...
  }, [editedTalk.weather, setEditedTalk]);
  React.useEffect(() => {
  ...
  }, [currentUser, editedTalk.supervisor, editedTalk.supervisorEmail, setEditedTalk]);

  const persist = async (next: ToolboxTalk, nextStep = step): Promise<boolean> => {
  ...
  const updateStructured = (content: StructuredTalkContent) => {
    const changed = (['i', 'hazards', 'practices', 'ppe', 'sif', 'manual', 'q'] as const).filter(key => JSON.stringify(structured?.[key]) !== JSON.stringify(content[key]));
    const next = { ...content, citations: (content.citations || []).map(c => ({ ...c, sections: c.sections.filter(key => !changed.includes(key as typeof changed[number])) })).filter(c => c.sections.length) };
    const strip = (text: string) => text.replace(/\(?\s*(?:29\s*CFR\s*)?1926\.\d+(?:\([a-zA-Z0-9]+\))*\s*\)?/g, '');
    for (const key of changed) {
      if (key === 'i') next.i = strip(next.i);
      else next[key] = next[key].map(strip);
    }
    setStructured(next); setEditedTalk(current => ({ ...current, content: JSON.stringify(next) }));
  };
  ...
  const writeItUp = async () => {
    ...
    try {
      ...
      const result = await response.json() as { content?: string; harness?: unknown }; if (!result.content) throw new Error('Writing assistance returned no record. Try again or type the record yourself.');
      const harness = useHarnessV2 ? parseHarnessReview(result.harness) : undefined;
      if (useHarnessV2 && !harness) throw new Error('The V2 review checks were missing. Please try again or write the record yourself.');
      const content = parseStructuredTalkContent(result.content) || makeStructured(result.content); const next = { ...clearApproval(editedTalk), title: editedTalk.title || `Safety Talk: ${notes.trim()}`, content: JSON.stringify(content), notes, drafted: true, approved: false, harness };
      setStructured(content); setEditedTalkState(next); await persist(next, 1); logger.logEvent(currentUser?.id || '', editedTalk.id, 'talk_generated', { latency_ms: logger.getElapsedTime(`ai_generation_${editedTalk.id}`), generator: useHarnessV2 ? 'harness_v2' : 'v1' });
  ...
  const chooseTemplate = async (id: string) => {
    ...
    const content = parseStructuredTalkContent(template.content) || makeStructured(template.content); const next = { ...clearApproval(editedTalk), harness: undefined, title: template.title, content: JSON.stringify(content), notes: notes || template.title, drafted: true, approved: false };
    setStructured(content); setEditedTalkState(next); await persist(next, 1); logger.logEvent(currentUser?.id || '', editedTalk.id, 'task_selected', { source: 'template_selection', template_id: id });
  };
  const proceed = async () => {
    if (sending || drafting || saveInFlight.current) return;
    setError('');
  ...
  return <main className="mx-auto max-w-[760px] px-5 pb-32 pt-7">
    ...
    {editedTalk.content && <aside aria-label="V2 draft review" className="mt-5 border border-rule bg-sheet p-4 text-sm text-ink-body">
      <p className="font-semibold">V2 draft review · {editedTalk.harness?.retrieval.status === 'grounded' ? `${editedTalk.harness.retrieval.sourceCount} OSHA sources retrieved` : 'OSHA evidence unavailable'}</p>
      <p className="mt-2">These automated checks are not a safety approval. Verify the draft and its citations before signing.</p>
      {reviewMessages.length > 0 && <div role="alert" className="mt-3 text-stop-text"><p className="font-semibold">Review required</p><ul className="mt-2 list-disc pl-5">{reviewMessages.map(message => <li key={message}>{message}</li>)}</ul></div>}
    </aside>}
