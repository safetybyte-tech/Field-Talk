  ...
  return <main className="mx-auto max-w-[760px] px-5 pb-32 pt-7">
    ...
    {step === 1 && editedTalk.content && reviewMessages.length > 0 && <details className="mt-5 border border-rule bg-sheet text-sm text-ink-body">
      ...
      <div className="px-4 pb-4">
        <p>Check these items against the work before signing. Automated checks are not a safety approval.</p>
        <ul className="mt-3 list-disc space-y-2 pl-5">{reviewMessages.map(message => <li key={message}>{message}</li>)}</ul>
      </div>
    </details>}
